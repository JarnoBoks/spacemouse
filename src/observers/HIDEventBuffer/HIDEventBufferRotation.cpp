...
void HIDEventBufferRotation::update(IObservable *kinVectorRotation) {
    ...
    const int16_t axisvalue = static_cast<KnobAxis *>(kinVectorRotation)->getFinValue(); // Final value of the rotation axis.
    ...
    const uint8_t idx = (static_cast<KnobAxis *>(kinVectorRotation)->getType() - MotionVector_t::ROTX) * 2; // Message index for the rotation axis.
