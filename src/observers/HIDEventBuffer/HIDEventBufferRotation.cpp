...
void HIDEventBufferRotation::update(IObservable *knobAxis) {
    if (!knobAxis) {
        return;
    }

    const int16_t axisvalue = static_cast<KnobAxis *>(knobAxis)->getFinValue(); // Final value of the rotation axis.
    ...
    const uint8_t idx = (static_cast<KnobAxis *>(knobAxis)->getType() - MotionVector_t::ROTX) * 2; // Message index for the rotation axis.
