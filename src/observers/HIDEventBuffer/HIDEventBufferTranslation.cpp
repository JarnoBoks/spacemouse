...
void HIDEventBufferTranslation::update(IObservable *knobVectorTranslation) {
    if (!knobVectorTranslation) {
        return;
    }

    const int16_t axisvalue = static_cast<KnobTranslation *>(knobVectorTranslation)->getFinValue(); // Final value of the translation axis.
    ...
    const uint8_t idx = (static_cast<KnobTranslation *>(knobVectorTranslation)->getType() - MotionVector_t::TRANSX) * 2; // Message index for the translation axis.
