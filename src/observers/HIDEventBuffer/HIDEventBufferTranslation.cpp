...
void HIDEventBufferTranslation::update(IObservable *kinVectorTranslation) {
    ...
    const int16_t axisvalue = static_cast<KinematicsAxis *>(kinVectorTranslation)->getFinValue(); // Final value of the translation axis.
    ...
    const uint8_t idx = (static_cast<KinematicsAxis *>(kinVectorTranslation)->getType() - MotionVector_t::TRANSX) * 2; // Message index for the translation axis.
